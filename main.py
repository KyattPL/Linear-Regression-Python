...
if __name__ == "__main__":
    ...
    fig = plt.figure(
        figsize=(12, 6), num='Zadanie najmniejszych kwadratow dla N=8')

    for i in range(8):
        w, err = least_squares(data['x_train_8'], data['y_train_8'], i)
        y_model = polynomial(x_plot, w)
        sub = fig.add_subplot(2, 4, i + 1)
        plot_model(data['x_train_8'], data['y_train_8'],
                   x_plot, y_obj, y_model)
    ...
    fig = plt.figure(
        figsize=(12, 6), num='Zadanie najmniejszych kwadratow dla N=50')

    for i in range(8):
        w, err = least_squares(data['x_train_50'], data['y_train_50'], i)
        y_model = polynomial(x_plot, w)
        sub = fig.add_subplot(2, 4, i + 1)
        plot_model(data['x_train_50'], data['y_train_50'],
                   x_plot, y_obj, y_model)
        sub.set_title("M = {}".format(i))
    ...
    fig = plt.figure(
        figsize=(6, 5), num='Selekcja modelu dla parametru regularyzacji')
    sub = fig.add_subplot(1, 1, 1)
